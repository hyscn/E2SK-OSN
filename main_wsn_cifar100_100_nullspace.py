            ...
            for key in consolidated_masks.keys():
                ...
                if (hasattr(getattr(model, module_name), module_attr)):
                    if (getattr(getattr(model, module_name), module_attr) is not None):
                        getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 1] = 0.5 * getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 1]
        optimizer.step()


def trian_pre_mask(args,model,device,x,y,optimizer,criterion,task_id,consolidated_masks):
    grad = []
    pre_mask_model = get_model(model)
    ...
    #             if (getattr(getattr(model, module_name), module_attr) is not None):
    #                 with torch.no_grad():
    #                     getattr(getattr(model, module_name), module_attr)[consolidated_masks[key] == 0] =  0   
    model.train()
    r = np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        if (hasattr(getattr(model, module_name), module_attr)):
            if (getattr(getattr(model, module_name), module_attr) is not None):
                grad.append(getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 0])
    return grad 
...
def get_correlation_mask(model,xtrain,ytrain,task_id,feature_pre,device):
    feature_current = []
    steps = 1
    ...
    if task_id == 0:
        for i in range(len(model.map)):
            ...
            if i < 3:
                ...
                act = model.act[act_keys[i]].detach().cpu().numpy()
                for kk in range(bsz):
                    for ii in range(s):
                        for jj in range(s):
                            mat[:,k]=act[kk,:,ii:ksz+ii,jj:ksz+jj].reshape(-1) 
                            k +=1
                feature_current.append(act)
            else:
                act = model.act[act_keys[i]].detach().cpu().numpy()
                activation = act[0:bsz].transpose()
                feature_current.append(activation)
    else:
        for i in range(len(model.map)):
            bsz = batch_list[i]
            k = 0 
        ...
        output_fea_pre = feature_pre[-1]
        output_fea_cur = feature_current[-1]
        sz1 = output_fea_cur.shape[0]
        dist = np.sqrt(np.sum(np.square(output_fea_cur - output_fea_pre))) / sz1
        #exp_dist = (1.0 / (1 + np.exp(-dist)) - 0.52) * 100
        exp_dist = 1.0 / (1 + np.exp(-dist))
        correlation_dist = exp_dist

    return feature_current, correlation_dist


def train_correlation(args,model,optimizer,criterion,device,xtrain,ytrain,task_id,mask_per_task):
    steps = 1
    model.train()
    r=np.arange(xtrain.size(0))
    np.random.shuffle(r)
    ...
    for i in range(0,len(r),args.batch_size_train):
        if ((i+args.batch_size_train) <= len(r)): 
            ...
            b = r[i:]
    example_data,target = xtrain[b].to(device),ytrain[b].to(device)
    if task_id == 0:   
        for i in range(steps):
            optimizer.zero_grad()
            example_out = model(example_data,task_id,mask=mask_per_task[task_id],mode="train")
        ...
        for i in range(steps):
            optimizer.zero_grad()
            example_out_pre = model(example_data,task_id,mask=mask_per_task[task_id - 1],mode = "train")
            example_out_current = model(example_data,task_id,mask=mask_per_task[task_id],mode = "train")
            loss = criterion(example_out_current,target)
            loss.backward()
        return example_out_pre,example_out_current

def compute_correlation(out_pre,out_cur,task_id):
    dist = torch.norm(out_cur[task_id - 1] - out_pre[task_id - 1])
    return dist
...
def main(args):
    ...
    for k, ncla in taskcla:
        ...
        # # get correlation
        # feature_pre, dist = get_correlation_mask(model,xtrain,ytrain,task_id,feature_pre,device)
        ...
        if task_id == 9:
            print('mask_9:',per_task_masks[task_id])
         
        # # Compute Correlation
        # print ('-'*40)
        # if task_id == 0:
        #     out_pre = train_correlation(args,model,optimizer,criterion,device,xtrain,ytrain,task_id,per_task_masks)
        #     out_pre_list.append(out_pre)
        # else:
        #     out_pre,out_cur = train_correlation(args,model,optimizer,criterion,device,xtrain,ytrain,task_id,per_task_masks)
        #     out_pre_list.append(out_pre)
        #     out_cur_list.append(out_cur)
        
        # if task_id != 0:
        #     dist = compute_correlation(out_pre,out_cur,task_id)
