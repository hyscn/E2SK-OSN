            ...
            for key in consolidated_masks.keys():
                ...
                if (hasattr(getattr(model, module_name), module_attr)):
                    if (getattr(getattr(model, module_name), module_attr) is not None):
                        getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 1] = 0 * getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 1]
        optimizer.step()
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        if consolidated_masks is not None and consolidated_masks != {}: # Only do this for tasks 1 and beyond
            ...
            for key in consolidated_masks.keys():
                ...
                if (hasattr(getattr(model, module_name), module_attr)):
                    if (getattr(getattr(model, module_name), module_attr) is not None):
                        getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 0] =  getattr(getattr(model, module_name), module_attr).grad[consolidated_masks[key] == 0]
        kk = 0 
...
def main(args):
    ...
    sparsity_list = np.random.rand(20)
    task_sparsity_id = 0
    ...
    for k,ncla in taskcla:
        ...
        #             if (getattr(getattr(model, module_name), module_attr) is not None):
        #                 with torch.no_grad():
        #                     getattr(getattr(model, module_name), module_attr)[consolidated_masks[key] == 1] = 0
        ...
        if task_id == 0:
            ...
            #                     getattr(getattr(model, module_name), module_attr)[consolidated_masks[key] == 1] = 0
            #                     getattr(getattr(model, module_name), module_attr)[consolidated_masks[key] == 1] += 0
            mat_list = get_representation_matrix_super_cifar100(model,device,xtrain,ytrain)
