                ...
                if 'last' in key_split or len(key_split) == 2:
                    ...
                    if hasattr(curr_module, module_attr):
                        if getattr(curr_module, module_attr) is not None:
                            getattr(curr_module, module_attr).grad[consolidated_masks[key] == 1] = 0 * getattr(curr_module, module_attr).grad[consolidated_masks[key] == 1]
...
def main(args):
    ...
    for k in range(num_tasks):
        ...
        #         data = v_x.to(device)
        #         target = v_y.to(device)
        #     mat_list = get_representation_matrix_tiny(model,device,data,target)
        #     print(mat_list)
        # Print Sparsity
